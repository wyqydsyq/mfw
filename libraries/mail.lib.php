 ...
 * mail class
 * 
 * provides mail related functions and manipulation
 */
if (!class_exists('mail')) {

	class mail extends mfw {
		...
		public function compose($to, $subject = '', $body = '', $from = '', $attachments = array(), $template = true) {
			...
			if ($template) {
				ob_start();
				extract($this->template_set);
				include($this->root_url . $this->router->base . $this->template);
				$body = ob_get_clean();
			}
			...
			if (!empty($attachments)) {
				foreach ($attachments as $attachment) {
					$file = $this->root_url . $attachment;
					$mime = new finfo(FILEINFO_MIME, "/usr/share/misc/magic");
					$mime = $mime->file($file);
					$this->Attach($file, $mime);
				}
			}
		...
		function Attach($filename, $filetype = "", $disposition = "attachment") {
